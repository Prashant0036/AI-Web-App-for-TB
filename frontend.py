...
# UI helper to show results
from utils.assesment_UI import show_result_ui

# local processing (DB insert + LLM call)
from services.processor import process_patient_sync
...
st.markdown("<h4 style='text-align: center;'>🧾 Fill the details below to get a free TB assessment</h4>", unsafe_allow_html=True)
...
with col2:
    wt = st.number_input("Weight (kg) — optional", min_value=0.0, max_value=300.0, step=0.1)
    ht = st.number_input("Height (cm) — optional", min_value=0.0, max_value=250.0, step=0.1)
    sys_bp = st.number_input("Systolic BP (mmHg) — optional", min_value=0, max_value=300, step=1)
with col3:
    dia_bp = st.number_input("Diastolic BP (mmHg) — optional", min_value=0, max_value=200, step=1)
    temp = st.number_input("Temperature (°F)", min_value=90.0, max_value=110.0, step=0.1, value=98.6)
    rp_rate = st.number_input("Respiratory rate (breaths/min) — optional", min_value=0, max_value=60, step=1)
...
# ---- Footer ----
st.markdown("<hr><center>The outputs are generated by an AI model and may not always be accurate or reliable. Please consult qualified professionals for verification.</center>", unsafe_allow_html=True)
...
if st.button("Submit"):
    if not name or age == 0:
        ...
        with st.spinner("Submitting details and analysing... this may take a while ⏳"):
            try:
                # --- SET SECRETS into ENV so LLM and DB modules (which read os.getenv) work ---
                # Streamlit provides secrets via st.secrets dict when deployed.
                if "GEMINI_API_KEY" in st.secrets:
                    os.environ["GEMINI_API_KEY"] = st.secrets["GEMINI_API_KEY"]
                if "DB_URI" in st.secrets:
                    os.environ["DB_URI"] = st.secrets["DB_URI"]

                # Process locally (in same container) - inserts to DB & calls LLM
                backend_response_json = process_patient_sync(patient_data)

                st.success("Details submitted successfully ✅")
                show_result_ui(backend_response_json)
            except Exception as e:
                st.error("Request failed while processing. Please try again or contact admin.")
